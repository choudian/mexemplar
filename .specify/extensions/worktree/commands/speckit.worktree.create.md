...
When this command is invoked as a `before_specify` hook, the user input is the **feature description** the user typed after `/speckit.specify`. Always treat it as new-feature mode in that context.
...
3. **Create branch + worktree atomically** from the main repository root (do **not** `cd` into the worktree to do this):
   ...
   - The main worktree's branch and working tree are unchanged — verify with `git -C . rev-parse --abbrev-ref HEAD` (should match the original branch).
