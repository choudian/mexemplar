...
- **Segment (片段)**: One continuous open-to-close stretch of conversation; the unit of distillation analysis. One Session can contain multiple Segments. A Segment carries its own status: `pending` (sealed, awaiting distillation — distinct from `failed`), `distilling` (analysis in progress), `completed` (distillation succeeded and committed any produced Memory Entries), `failed` (exhausted retries, including the one all-empty retry path, without producing a usable active-zone entry set — shown in management UI as "distillation failed, manual retry available"; a user-triggered retry returns the Segment to `pending`).
