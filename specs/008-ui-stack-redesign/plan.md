...
| V. Living Docs & Spec-Driven Delivery | Are required active docs identified for update, and are temporary notes confined to `docs/local/`? | This feature owns `specs/008-ui-stack-redesign/*`; implementation must update `docs/ARCHITECTURE.md`, `docs/PROJECT_CONSTRAINTS.md`, `AGENTS.md`, packaging docs, and user-facing install/run docs when code reality changes. The local UI stack decision input is recorded in `docs/local/_archive/design-drafts/2026-05-10-ui-tech-stack-decision.md`; durable acceptance traceability lives in `control-inventory.md`. | PASS |
