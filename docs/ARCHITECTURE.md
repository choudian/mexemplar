...
- Task 执行节点状态：`pending_dispatch / running / delivered / suspended / completed / skipped / abandoned / cancelled`。”等待裁定”由 `delivered` 显式表达（执行体交了活、等主助理验收）；`assistant_task_adjudications` 记录裁定详情。父侧裁定支持认可、打回、放弃；只有根图终态失败才桥接到 `assistant_run_failures`。
...
**临时子代理可唤回**：办公助理派出的临时子代理（`resumable_on_failure=True`）撞迭代上限或遇可恢复的 LLM 失败（账户配额/限流/网络）时，AgentLoop 返回 `ResultType.PAUSED` 并将会话置 `suspended`，Orchestrator 据此回传带 `subagent_id` 的可唤回句柄并记一条内部 `assistant_delegation_paused` workflow transition（非公开 UI 事件）。主助理以及固定 executor 专员都可用 `inspect_subagent`（只读概览、零模型调用）与 `continue_subagent`（凭 `subagent_id` 从持久化会话历史恢复续跑，经归属校验）调度；planner 和 ephemeral 不获得这两个工具。专员的隔离子代理在首次启动前物化 `EffectiveSubagentScope`，把模型请求限制在专员实际拥有的内置/动态/组合授权和同一 `workspace_root` 内；同一次专员运行中的续跑复用该不可变快照并只按当前发布状态收紧。该续跑快照 V1 不持久化；续跑请求遇到工具闭包重建或 child id 不匹配时 fail-closed 并要求 `ask_parent` 升级。`inspect_subagent` 不消费权限快照，仍由持久父子归属校验保护。MCP 与执行器协作工具沿用各自的 ambient 授权模型，不进入这份快照。同一次专员运行内可以先后启动多个隔离子代理，暂停 child 不占运行并发槽；`delegate_to_subagent` / `continue_subagent` 都显式标记为有副作用且不可并发，配合同步 handler 与 AgentLoop 串行分区，保证 child 启动调用不重叠。该保证只覆盖同一次 specialist 执行实例，不表示同一 `specialist_id` 在所有 fallback 路径上全局唯一。工作历史零丢失且不引入主助理直接执行路径；不可恢复的 LLM 失败仍返回 `ERROR`。
