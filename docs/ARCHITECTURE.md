...
- **公开 UI 事件**（Registry + projector，payload 走 009 allowlist 脱敏）：`assistant.activity`（过程时间线，按 `subagentId` 归类）、`assistant.subagent`（子任务卡片壳与状态）；`assistant.progress.status` 新增 `cancelled` 取值，运行中 payload 带 `runId` 供停止请求绑定当前代际。非助理 agent_type 在 projector 处过滤，零 UI 噪声。
