...
- 运行中取消必须使用带 reason 和回调的 `CancelToken`：`user-cancel` / `sibling_error` 要中断当前可取消的 LLM HTTP 请求、终止同步命令整棵进程树，并在执行体退出时清理该 session 拥有的后台进程；不得关闭其他并发请求的共享 transport 或清理其他 session 的进程。被杀的当前工具必须补且只补一条 `{"outcome":"interrupted","note":"被中断，副作用状态未知"}` 结果，尚未开工的后续调用保持 pending；取消不自动回滚文件。
- 建图后绝对路径预检属于确定性 advisory：只按节点描述和执行体工作区根报告范围外风险，不得把风险清单冒充通过/否决结果；当前纯函数尚未接入调度入口。
...
- 启动外部 coding session 必须显式提供目标仓库的绝对路径；缺失、不存在、非 git 仓库或 HEAD 不可解析时一律 fail-closed，且不得留下 worktree、分支或 session 行。HEAD/目标分支 probe 只转换预期 `GitOperationError`；Git timeout/OSError 归一化为不含 OS 正文的 `GitProcessError` 并给出可行动提示，未知异常不得伪装成路径冲突或无提交。显式指向 Exemplar 自身仓库是允许的，不得恢复任何隐含 cwd 默认值。artifact 根在创建入口统一展开为绝对路径，handoff/plan/result/prompt 不得以相对路径持久化后交给已切换 cwd 的外部 CLI。
- artifact/handoff、worktree/分支与首次 session 写入属于同一启动补偿边界；Git 删除必须检查退出码并验证 worktree 注册、目录和分支均消失。补偿自身失败时必须返回带 session 标识的安全 cleanup-incomplete，不能静默宣称零残留；首次 session 写入与回读都无法确认结果时必须保留资源并返回预生成 session id，安全日志只记录该 id 与 create/lookup 异常类型。CLI 启动前必须先持久化 attempt reservation；adapter object 构造成功后，reservation 才可通过 `attempt_id + status=running + launch_started=false` 的 Repository CAS 原子翻为 true，factory 失败按 pre-spawn 关单，CAS 未命中不得 spawn。`launch_started` 只可由专用 CAS false→true：通用 Repository 更新不得写该字段，v33 UPDATE trigger 必须拒绝 true→false。每个 session 至多一个 running attempt，由数据库 partial unique index 兜底。domain value 校验其可见的 status/ownership 与 PID/创建时间组合；Repository、ORM checks 和 v33 幂等 insert/update triggers 另守 pre-spawn identity 等持久行不变量。spawn 后初始化或结果落库失败必须停止并确认进程。停止结果使用“确认已停止 / 确认先前退出 / 无法确认”三态契约；无法确认时持久化 `terminationUnconfirmed` 并保留 running/PID 管理权，普通 poll 终态不得替代进程树退出证明。running attempt 还必须持久化 PID 创建时间与 `termination_unconfirmed=true`；只有匹配 durable PID + 创建时间的确定终态可清除。v33 升级必须把既有 running 行回填为 unconfirmed；durable 创建时间缺失时不得从状态文件反向补齐身份。进程 registry 按完整身份与状态路径查找并以对象 CAS 删除，PID 复用不得覆盖或误删 owner；碰撞后的新进程若补偿终止失败，旧、新 owner 必须隔离保留。重启后只以 PID + 创建时间认领/终止，状态文件缺失、父进程消失或身份冲突一律保持 running/fail-closed；匹配的存活 PID 即使带旧 unconfirmed marker 也必须允许再次 stop。headless/interactive monitor 必须共用顶层守护边界，输出 reader 的满队列写入必须可取消，EOF/失败 sentinel 是 stream 完成权威，进程退出与 queue 瞬时为空不得提前判成功；任意异常与 wait 超时后的补偿都必须按整棵进程树终止和确认，无法确认时保留 registry 与 durable ownership。已确认退出但 terminal 首写失败时后续 poll 只有重放落盘成功才释放 owner。poll 观测失败保持 running，禁止按 interrupted 续跑；所有 running 观测写入使用 expected-status CAS，terminal 不得被迟到观测复活或清除 `finished_at`；failed/interrupted attempt、abandon/协议违规等显式 stop 与关联 session 投影必须在 Repository 单事务提交，成功 attempt 终态与随后 PLAN/RESULT 校验是可重试的两步投影。启动成功或失败的 command summary 均须遮蔽绝对路径；session 首次写入后的 plan/result artifact 路径不可变。
- `external_coding.worktree_root` 为相对路径时必须以目标仓库为基准展开；为绝对路径时保持该绝对根。键名与默认值 `.worktrees/coding` 不变。
- 外部 CLI 仍须以 argv 且 `shell=False` 启动；Windows 命令名先经 PATH/PATHEXT 解析。命令构造必须随本机 CLI 版本复核：Claude headless `stream-json` 携带所需 verbosity；Codex headless resume 的 exec 父参数置于 `resume` 前，并用 final-output 文件参数确定性落盘阶段 artifact。完整绝对路径不得进入命令摘要。
