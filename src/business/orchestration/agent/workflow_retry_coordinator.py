...
from .ports import AgentExecutionPort, ReviewStatePort
...
class WorkflowRetryCoordinator:
    def __init__(
        ...
        failure_tracker,
        session_store,
        agent_execution_port: AgentExecutionPort,
        review_state: ReviewStatePort,
        logger: Optional[logging.Logger] = None,
    ) -> None:
        self._failure_tracker = failure_tracker
        self._session_store = session_store
        self._agent_execution_port = agent_execution_port
    ...
    def retry_teaching(self, workflow_id: str) -> None:
        ...
        if failed_stage == AgentType.PM:
            self.retry_stage(
                ...
                record,
                AgentType.PM,
                fallback_action=lambda: self._agent_execution_port.start_analysis(
    ...
    def retry_stage(
        ...
        *,
        ...
        if session_id:
            self._agent_execution_port.run_agent(
                stage,
                None,
                workflow_id,
        ...
        if transition_event and transition_key:
            ...
            if payload:
                ...
                self._agent_execution_port.run_agent(stage, user_input, workflow_id)
                return
        ...
        if _depth >= 3:
            ...
            self._agent_execution_port.start_analysis(workflow_id, workflow_id)
            return
