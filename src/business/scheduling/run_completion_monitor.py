...
Blinker receiver 只是同步 observer：评估异常在 receiver 边界记录并隔离，不得反向
打断 task collaboration 发送方；主动调用 ``evaluate_session`` 时异常仍正常传播。
"""
...
class RunCompletionMonitor:
    ...
    - ``get_graph_snapshot(session_id) -> TaskGraphSnapshot | None``：可选；未注入时走
      ``AssistantTaskRepository`` + ``TaskCollaborationService`` 的内部权威查询。
    """
    ...
    def __init__(
        ...
        *,
        ...
        has_pending_reentry: Callable[[str], bool] | None = None,
    ...
    def _on_graph_terminal(self, _sender, **kwargs) -> None:
        ...
        if not session_id or not graph_id:
            return
        self._evaluate_session_from_event(
            session_id,
    ...
    def _evaluate_session_from_event(self, session_id: str, *, event_name: str) -> None:
        """Best-effort observer 边界；绝不把评估异常抛回核心事件发送方。"""
        try:
            self.evaluate_session(session_id)
        except Exception:
            logger.error(
                "RunCompletionMonitor: %s evaluation failed for session %s",
                event_name,
                session_id,
                exc_info=True,
            )
    ...
    def evaluate_session(self, session_id: str) -> Optional[dict[str, Any]]:
        """判静默并写 run 终态。返回 run 投影 dict，或 None（无关联 run / 未静默 / 已终态）。

        供 worker 周期调用与 reentry finally 主动调用。
        """
        ...
        with self._evaluation_lock:
            with self._run_repo_scope() as run_repo:
                return self._evaluate_session_locked(sid, run_repo)
    ...
    def _evaluate_session_locked(
        ...
        run = run_repo.get_by_session(sid)
        if run is None:
            return None  # 非 scheduled 会话或无 run
        ...
        graph_state = self._quiescent_graph_state(sid)
        if graph_state is None:
            return None
        _, all_completed = graph_state

        if all_completed:
            summary = self._extract_summary(sid)
    ...
    def mark_waiting_user(self, session_id: str) -> Optional[dict[str, Any]]:
        """scheduled 主助理反问时原子落 ``waiting_user`` 并发一次接管通知。"""
        sid = (session_id or "").strip()
        if not sid:
            return None
        with self._evaluation_lock:
    ...
    def mark_failed(
        ...
        *,
        ...
        """明确的 assistant 失败/停止结果直接结束 scheduled run。"""
        sid = (session_id or "").strip()
        if not sid:
            return None
        with self._evaluation_lock:
    ...
    def _quiescent_graph_state(self, session_id: str) -> tuple[bool, bool] | None:
        ...
        try:
            ...
            return None
        if self._has_pending_reentry_now(sid):
            return None
        graph_state = self._compute_graph_state(sid)
        if graph_state is None:
            return None
    ...
    def _has_pending_reentry_now(self, session_id: str) -> bool:
    ...
    def _compute_graph_state(self, session_id: str) -> tuple[bool, bool] | None:
        known, snapshot = self._get_snapshot(session_id)
        if not known:
            return None
        if snapshot is None:
    ...
    def _get_snapshot(self, session_id: str) -> tuple[bool, Any | None]:
        ...
        if self._get_graph_snapshot is not None:
            try:
                return True, self._get_graph_snapshot(session_id)
            except Exception:
                logger.error(
        ...
        graph_id_known, graph_id = self._lookup_current_graph_id(session_id)
    ...
    def _lookup_current_graph_id(self, session_id: str) -> tuple[bool, Optional[str]]:
        try:
            ...
            with AssistantTaskRepository() as repo:
                return True, repo.get_current_graph_id(session_id)
        except Exception:
            logger.error(
        ...
        try:
            with self._message_repo_scope() as message_repo:
                text = message_repo.get_latest_assistant_text(
                    session_id, max_length=SUMMARY_MAX_CHARS
                )
            return text or ""
        except Exception:
            logger.warning(
