"""
Brain Context Builder - 异步构建多分区上下文，注入 assistant prompt
...
# 复合评分权重（与 retrieval_service 保持一致）
_HOT_RELEVANCE_WEIGHT = 0.35
...
# 新条目探索加分阈值
_EXPLORATION_LOADED_THRESHOLD = 3
...
class BrainContextBuilder:
    ...
    def __init__(self, repo=None, specialist_repo=None, config=None):
        ...
        if config is not None:
            self._config_int(config, "get_brain_injection_hot_zone_top_n", 20)
    ...
    def build_context(
        ...
        subconscious_top_n = self._config_int(
            config, "get_brain_injection_subconscious_top_n", 10,
        )

        # 持久区：全量注入 active 条目
        persistent_rows = self._entries_for_zone(
            repo, Zone.PERSISTENT.value, status=EntryStatus.ACTIVE.value, limit=None,
        )
        ...
        hot_rows = self._entries_for_zone(
            repo, Zone.HOT.value, status=_HOT_VISIBLE_STATUSES,
        )
        scored_hot = []
        ...
        try:
            ...
            specialists = specialists_result[0] if isinstance(specialists_result, tuple) else specialists_result
        ...
        logger.info(
            ...
            session_id, len(persistent_entries), len(hot_entries),
            len(specialist_list), is_cold_start, is_revived_session,
        )
    ...
    def format_context_for_prompt(self, context: BrainContext) -> str:
        ...
        if context.is_cold_start:
            sections.insert(0, "（这是你与用户的首次对话或大脑记忆为空。请主动了解用户，可以问 1-2 个问题来建立初步了解。）")
