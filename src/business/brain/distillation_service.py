...
class DistillationService:
    ...
    def _build_distillation_prompt(self, phase: str = "p1") -> str:
        base = (
            ...
            "4. **忠实原意**：记忆必须忠实于用户实际表达，不能加码、不能升级\n"
            "   ✓ 用户说「这本书水分没那么大」→ 记「用户认为书籍类内容不适合激进压缩」\n"
            "   ✗ 用户说「解析一下」→ 记「用户习惯先要完整结构拆解再决定是否采用」← 加码\n"
            "   ✗ 用户问「有图吗？」→ 记「用户确认了图数据结构」← 疑问变确认\n"
            "   ✗ 用户说「水分不大」→ 记「宁可笨重也不接受简洁」← 单点质疑升级为绝对偏好\n\n"
            "## 常见错误（避免）\n"
            ...
            "- 把助理的发挥当用户的主张：「用户认为压缩会丢推理过程和案例细节」← 如果「推理过程、案例细节」是助理补充的而非用户原话，不得归为用户的观点\n"
            "- 把助理的行为当用户的习惯：「用户本能地做横向对比」← 对比是助理做的，用户只是追问，不能把助理行为归为用户倾向\n\n"
            "## 分区规则\n"
            "### hot_zone（热区）\n"
        ...
        base += (
            ...
            "## 去重校验\n"
            "输出前检查：是否有两条记忆本质上说的是同一件事？如果多条记忆的核心观察相同，只保留表述最准确的那一条，其余删掉。\n"
            "✓ 保留「LLM Wiki vs codebase-memory-mcp：前者有损压缩，后者零损耗」\n"
            "✗ 删掉「用户质疑压缩丢信息」「用户对保真度有偏好」「压缩后只留结论」——三条说的是同一件事\n"
        )
        feedback_guidance = self._format_feedback_guidance()
        if feedback_guidance:
            base += "\n近期用户反馈信号（参考，不要逐条转为记忆）\n" + feedback_guidance + "\n"
        base += "\n没有值得提取的，所有分区留空数组——这完全正常，不要为了填满而提取。"
        return base
    ...
    def run_subconscious_distillation(self, llm_client=None) -> int:
        ...
        response = llm_client.chat_with_tools(
            messages=[
                {
                    "role": "system",
                    "content": self._build_subconscious_prompt(),
                },
    ...
    def _build_subconscious_prompt(self) -> str:
        """构建潜意识 distillation 的 system prompt。

        证据数量与跨对话质量规则属于 prompt 引导（advisory），不是 schema 或业务层硬校验
        ——见 ``src/CLAUDE.md``「Brain Service 约束」。模型仍可能违规输出；若需要硬保证，
        应在 ``SUBCONSCIOUS_DISTILLATION_TOOL_SCHEMA`` 增加证据字段并在验证器层校验，
        而非依赖这里的 prompt 文本。
        """
        return (
            ...
            "- 从同一段对话的多条记忆推导出的「模式」（那只是对一次事件的反复解读，不是跨对话的模式）\n"
            "  ✗「用户本能地拉入竞品做横向对比」← 证据全部来自同一段对话，不是跨对话模式\n"
            "- 把助理的行为模式归为用户的习惯\n"
            "  ✗「用户本能地做横向对比」← 对比是助理做的，用户只是追问\n\n"
            "## 证据自检\n"
            "每条潜意识记忆的 reason 必须明确写出：证据分别来自哪几次不同的对话（用对话时间或主题区分）。\n"
            "如果所有证据都来自同一段对话，这条记忆不合格，不要输出。\n\n"
            "## 输出\n"
