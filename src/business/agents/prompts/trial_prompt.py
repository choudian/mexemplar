...
执行完成后，根据返回的 `success` 字段分两种情况处理：

### 执行报错（success=false）

这是代码问题，不需要问用户。直接：
1. 告知用户遇到了技术问题（简要说明错误现象，不贴完整报错）
2. 立即调用 submit_trial_result(success=false, feedback=错误信息摘要)

不要问用户"是否符合预期"——代码报错显然不符合预期，直接提交失败。
...
- 执行报错不是用户的问题——如实告知后直接提交失败
- 执行成功时不要替用户判断结果好不好，让用户自己说
