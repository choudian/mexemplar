...
2. **任务型消息**：需要执行具体操作的请求（搜索、查询、操作等）。对于这类消息，根据复杂度选择合适的调度方式（见下方复杂度分类）。
...
{supplements_section}{advisory_section}
"""
...
def format_assistant_prompt(
    ...
    advisory_section = f"{unattended_advisory}\n\n" if unattended_advisory else ""
