...
class PendingToolRepository(RawSQLRepository):
    """待试用工具仓库"""
    ...
    def get_by_id(self, pending_tool_id: str) -> Optional[PendingTool]:
        """根据ID获取待试用工具"""
        row = self._fetch_one("SELECT * FROM pending_tools WHERE pending_tool_id = ?", (pending_tool_id,))
        if row:
    ...
    def delete(self, pending_tool_id: str) -> bool:
        """删除待试用工具"""
        cursor = self._execute("DELETE FROM pending_tools WHERE pending_tool_id = ?", (pending_tool_id,))
...
class ToolTrialRepository(RawSQLRepository):
    """工具试用记录仓库"""
    ...
    def get_by_id(self, trial_id: str) -> Optional[ToolTrial]:
        """根据ID获取试用记录"""
        row = self._fetch_one("SELECT * FROM tool_trials WHERE trial_id = ?", (trial_id,))
        if row:
    ...
    def delete(self, trial_id: str) -> bool:
        """删除试用记录"""
        cursor = self._execute("DELETE FROM tool_trials WHERE trial_id = ?", (trial_id,))
        deleted = cursor.rowcount > 0
        if deleted:
