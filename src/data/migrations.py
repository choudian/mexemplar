...
def migrate_to_v40(engine):
    """迁移到版本 40：``assistant_tasks.status`` 的 ``failed`` 改名为 ``abandoned``。

    这不是加一个格子，是**给同一个格子换一个说真话的名字**。

    能进这个格子的只有两条路，两条都是明确拍板：裁定选 abandoned，或主助理调
    ``abandon_request_graph`` 放弃整个请求。系统侧的失败根本不走这里——执行体崩了
    写 attempt 表并建待裁定，租约过期回 ``pending_dispatch`` 重派。

    叫 ``failed`` 时，读到它的人（主助理、看板、日志）会以为"系统判它死了，也许
    该重试"，而真相是有人已经决定放弃。语义和处置正好相反。

    存量行在 ``INSERT ... SELECT`` 里就地转换，**不能先插入再 UPDATE**：新表的
    CHECK 已经不认 ``failed``，先插会被当场拒绝。
    """
    try:
        with engine.begin() as conn:
            exists = conn.execute(
            ...
            if exists is None:
                logger.info("迁移到版本 40：assistant_tasks 表不存在，跳过 status 改名")
            ...
            conn.execute(
                text(
                    """
                    CREATE TABLE assistant_tasks_v40 (
                        task_id TEXT PRIMARY KEY,
                        graph_id TEXT NOT NULL,
                        root_task_id TEXT,
                        parent_task_id TEXT,
            ...
            conn.execute(
                text(
                    """
                    INSERT INTO assistant_tasks_v40 (
                        task_id, graph_id, root_task_id, parent_task_id,
                        session_id, user_message_sequence, title, description,
                        status, suspend_reason, waiting_on, assignee_type,
                        assignee_id, owner_session_id, capability_scope,
                    ...
                    SELECT
                        ...
                        suspend_reason, waiting_on, assignee_type,
                        assignee_id, owner_session_id, capability_scope,
                        graph_version, task_version, created_at, updated_at,
                        completed_at, failed_at, cancelled_at,
                        requires_confirmation, workspace_root
            ...
            conn.execute(text("DROP TABLE assistant_tasks"))
            conn.execute(text("ALTER TABLE assistant_tasks_v40 RENAME TO assistant_tasks"))
            for index_sql in (
                "CREATE INDEX idx_assistant_tasks_graph_status "
                "ON assistant_tasks(graph_id, status)",
                "CREATE INDEX idx_assistant_tasks_graph_parent "
        ...
        logger.error(f"迁移到版本 40 失败: {e}")
        raise
    logger.info("迁移到版本 40 完成：assistant_tasks.status failed → abandoned")


def migrate_to_v41(engine):
    """迁移到版本 41：允许任务以 ``quota_exhausted`` 等待用户充值。"""
    try:
        with engine.begin() as conn:
            exists = conn.execute(
                text(
            ...
            if exists is None:
                logger.info("迁移到版本 41：assistant_tasks 表不存在，跳过暂停原因扩展")
            ...
            conn.execute(
                text(
                    """
                    CREATE TABLE assistant_tasks_v41 (
                        task_id TEXT PRIMARY KEY,
                        graph_id TEXT NOT NULL,
                        root_task_id TEXT,
                        parent_task_id TEXT,
            ...
            conn.execute(
                text(
                    """
                    INSERT INTO assistant_tasks_v41 (
                        task_id, graph_id, root_task_id, parent_task_id,
                        session_id, user_message_sequence, title, description,
                        status, suspend_reason, waiting_on, assignee_type,
                        assignee_id, owner_session_id, capability_scope,
            ...
            conn.execute(text("DROP TABLE assistant_tasks"))
            conn.execute(text("ALTER TABLE assistant_tasks_v41 RENAME TO assistant_tasks"))
            for index_sql in (
                "CREATE INDEX idx_assistant_tasks_graph_status "
                "ON assistant_tasks(graph_id, status)",
                "CREATE INDEX idx_assistant_tasks_graph_parent "
        ...
        logger.error(f"迁移到版本 41 失败: {e}")
        raise
    logger.info("迁移到版本 41 完成：quota_exhausted")
...
def migrate_to_v46(engine):
    """迁移到版本 46：assistant_tasks.status 的 ``completed`` 改名为 ``done``。

    ⑥ 第二步：纯改名——``completed`` 的语义本就是"验收通过"（accept 裁定后写入），
    ``done`` 这个名字更准确。存量行在 ``INSERT ... SELECT`` 里就地转换（照 v40 模式）。

    ⚠️ 改完后 ``assistant_tasks.status = 'done'`` 跟 ``adjudication.delivered_status = 'done'``
    在库里和日志里长得一样，靠表名区分语义。
    """
    try:
        with engine.begin() as conn:
            ...
            if exists is None:
                logger.info("迁移到版本 46：assistant_tasks 表不存在，跳过")
                conn.execute(text("UPDATE schema_version SET version = 46"))
                return

            conn.execute(
                text(
                    """
                    CREATE TABLE assistant_tasks_v46 (
                        task_id TEXT PRIMARY KEY,
                        graph_id TEXT NOT NULL,
                        root_task_id TEXT,
                        parent_task_id TEXT,
                        ...
                        status TEXT NOT NULL DEFAULT 'pending_dispatch'
                            ...
                            CHECK (status IN (
                                'pending_dispatch', 'running', 'delivered', 'suspended',
                                'done', 'skipped', 'abandoned', 'cancelled'
                            )),
                        suspend_reason TEXT
                            CONSTRAINT ck_assistant_tasks_suspend_reason
                            CHECK (
            ...
            conn.execute(
                text(
                    """
                    INSERT INTO assistant_tasks_v46 (
                        task_id, graph_id, root_task_id, parent_task_id,
                        session_id, user_message_sequence, user_task_id,
                        title, description, status, suspend_reason, waiting_on,
                        assignee_type, assignee_id, owner_session_id, capability_scope,
                    ...
                    SELECT
                        ...
                        session_id, user_message_sequence, user_task_id,
                        title, description,
                        CASE WHEN status = 'completed' THEN 'done' ELSE status END,
                        suspend_reason, waiting_on,
                        assignee_type, assignee_id, owner_session_id, capability_scope,
                        graph_version, task_version, created_at, updated_at,
                        completed_at, failed_at, cancelled_at,
                        requires_confirmation, workspace_root
            ...
            conn.execute(text("DROP TABLE assistant_tasks"))
            conn.execute(text("ALTER TABLE assistant_tasks_v46 RENAME TO assistant_tasks"))
            for index_sql in (
                "CREATE INDEX idx_assistant_tasks_graph_status "
                "ON assistant_tasks(graph_id, status)",
                "CREATE INDEX idx_assistant_tasks_graph_parent "
