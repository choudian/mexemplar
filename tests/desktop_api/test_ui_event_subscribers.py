...
def test_two_subscribers_receive_same_event_without_stealing() -> None:
    ...
    for index in range(20):
        event_queue.publish_nowait(
            "settings.changed", {"reason": "settings_invalidated", "keys": [str(index)]}
        )
...
def test_private_resync_does_not_create_replay_gap_for_healthy_subscriber() -> None:
    ...
    next_event = event_queue.publish_nowait(
        "settings.changed", {"reason": "settings_invalidated", "keys": ["next"]}
    )
...
def test_reconnect_replays_buffered_same_session_events() -> None:
    first = event_queue.publish_nowait(
        "settings.changed", {"reason": "settings_invalidated", "keys": ["a"]}
    )
    second = event_queue.publish_nowait(
        "settings.changed", {"reason": "settings_invalidated", "keys": ["b"]}
    )

    sub = event_queue._subscribe(
        last_seen_sequence=first.sequence, event_session_id=first.sessionId
    )
...
def test_resync_control_events_have_unique_sequence_without_replay_gap() -> None:
    ...
    invalid_cursor = event_queue._subscribe(
        last_seen_sequence=first.sequence, event_session_id=None
    )
